...
<script>
	export default {
		...
		methods: {
			// 支付
			onPayTap(){
				uni.setStorageSync('paytable','huodongfenlei');
			...
			onAcrossTap(tableName,crossOptAudit,statusColumnName,tips,statusColumnValue){
				uni.setStorageSync('crossTable','huodongfenlei');
			...
			// 获取详情
			async init(){
				let res = await this.$api.info('huodongfenlei', this.id);
			...
			// 添加评论
			async onCommentTap() {
				this.$utils.jump(`../discusshuodongfenlei/add-or-update?refid=${this.id}`)
			},
			onSHTap() {
			...
			async onFinishTap() {
				...
				await this.$api.update('huodongfenlei', this.detail);
				this.$utils.msg('审核成功');
