...
<script>
	export default {
		...
		async onShow(options) {
			...
			let cleanType = uni.getStorageSync('discussdiscusstoupiaoxiangmuCleanType')
			if(cleanType){
				uni.removeStorageSync('discussdiscusstoupiaoxiangmuCleanType')
				this.mescroll.num = 1
		...
		methods: {
			// 支付
			onPayTap(){
				uni.setStorageSync('paytable','discusstoupiaoxiangmu');
			...
			onAcrossTap(tableName,crossOptAudit,statusColumnName,tips,statusColumnValue){
				uni.setStorageSync('crossTable','discusstoupiaoxiangmu');
			...
			// 获取详情
			async init(){
				let res = await this.$api.info('discusstoupiaoxiangmu', this.id);
			...
			async upCallback(mescroll) {
                let res = await this.$api.list('discussdiscusstoupiaoxiangmu', {
					page: mescroll.num,
					limit: mescroll.size,
					refid: this.id
				});
			...
			// 添加评论
			async onCommentTap() {
				this.$utils.jump(`../discussdiscusstoupiaoxiangmu/add-or-update?refid=${this.id}`)
			},
			onSHTap() {
			...
			async onFinishTap() {
				...
				await this.$api.update('discusstoupiaoxiangmu', this.detail);
				this.$utils.msg('审核成功');
